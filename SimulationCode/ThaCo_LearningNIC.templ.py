...
# Set save path
home = os.getcwd()
save_path = home +  '/../SimulationOutput/ThaCo_LearningNIC/set' + str(_isubset_) + '/'
...
rand = np.random.RandomState(subset)
# Shuffle data
shuffle = rand.permutation(len(labels_train0))
# ora label_red ha i dati mischiati
labels_train0 = [labels_train0[i] for i in shuffle]
feat_arr_train0 = [feat_arr_train0[i] for i in shuffle]

feat_arr_train = feat_arr_train0[N_RANKS*N_CLASSES*subset: N_RANKS*N_CLASSES* (subset+1)]
labels_train = labels_train0[N_RANKS*N_CLASSES*subset: N_RANKS*N_CLASSES* (subset+1)]

# labels_train = np.array(labels_train, dtype=np.float32)
labels_train = np.asarray(labels_train)
labels_train = labels_train.astype(int)

sio.savemat(save_path + 'labels_train.mat', {'labels_train': labels_train})
...
sio.savemat(save_path + 'labels_test.mat', {'labels_test': test_out})
...
W_noise_inp = 1600.
W_noise_exc = 1100.
W_noise_out = 1920. #1100. # diverso per leonardo 1920
W_noise_inh = 320.

Wmax_exc_exc = 150. # 150 o 20?
Wmax_inp_exc = 10.5
Wmax_exc_inp = 100. # per leonardo, oppure 5.#0.333
Wmax_exc_out = 67500.
Wmax_out_out = 20.
Wmax_out_exc = 0.#5.
...
nest.SetStatus(nest.GetConnections(source=neur_exc, target=neur_out), {'mu_minus': mu_minus, 'mu_plus': mu_plus, 'alpha': alpha})
...
ClassPrediction = np.zeros([np.int32(float(N_RANKS)/_interval_), N_TEST])
GroupPrediction = np.zeros([np.int32(float(N_RANKS)/_interval_), N_TEST])
ReadoutPrediction = np.zeros([np.int32(float(N_RANKS)/_interval_), N_TEST])
...
for i_cumul in range(0,N_RANKS*N_CLASSES, _interval_*N_CLASSES):
    ...
    sio.savemat(name, {'evt_exc': evt_exc_cnt_th, 'evt_th': evt_th_cnt_th})
    ...
    for i_test in range(n_test):  # per ogni esempio del test set
        ...
        ReadoutPrediction[np.int32(float(i_cumul)/N_CLASSES*interval), i_test] = i_out_num
        # ...fino a qui
        ...
        ClassPrediction[np.int32(float(i_cumul)/(N_CLASSES*interval)), i_test] = unsupervised_output_class
        ...
        GroupPrediction[np.int32(float(i_cumul)/(N_CLASSES*interval)), i_test] = unsupervised_output_group
    ...
    np.save(save_path + 'ClassPrediction', ClassPrediction)
    np.save(save_path + 'GroupPrediction', GroupPrediction)
    np.save(save_path + 'ReadoutPrediction', ReadoutPrediction)
