...
# Set save path
home = os.getcwd()
save_path = home +  '/Watson010/Std_' + str(_Std_) + '/alpha' + str(_alpha_) + '/b' + str(_b_) + '/tau' + str(_tau_) + '/inh' + str(_Winh_) + '/Rate_' + str(_rate_) + '/set' + str(_isubset_) + '/'
...
fn_train = 'Mnist_PreprocessingNew/mnist_training_features_3x3_Coding6_0.9_14.npy'
feat_arr_train0 = np.load(fn_train)
fn_test = 'Mnist_PreprocessingNew/mnist_test_features_3x3_Coding6_0.9_14.npy'
feat_arr_test = np.load(fn_test)


INPUT_SIZE = 3*3*9*coding_number

label_fn_train =  'Mnist_PreprocessingNew/mnist_training_labels_0.9_14.npy'
labels_train0 = np.load(label_fn_train)

label_fn_test = 'Mnist_PreprocessingNew/mnist_test_labels_0.9_14.npy'
...
W_noise_inp = 1600.
W_noise_exc = 110. * 10.#310.#2250.
W_noise_out = 320. *6.
W_noise_inh = 320. #/2.5 /2.

Wmax_exc_exc = 150. #150.#20.
Wmax_inp_exc = 10.5
Wmax_exc_inp = 100.#0.333
Wmax_exc_out = 67500.
Wmax_out_out = 20.
Wmax_out_exc = 0.#5.
...
for i_cumul in range(0, 5 * N_CLASSES, _interval_ * N_CLASSES):
    ...
    if i_cumul == 40: #quando vedo 5 esempi per categoria
        ...
        nest.Simulate(t_pause * 2)  # 5
        timetot = timetot + t_pause*2
        ...
        AccPreReadout = float(count_right_num) / float(i_test + 1.) * 100.
        AccPreClass = float(count_unsupervised_class_right) / float(i_test + 1.) * 100.
        AccPreGroup = float(count_unsupervised_group_right) / float(i_test + 1.) * 100.
        ...
        name = save_path + 'weights_PreSleep' + '.mat'
        ...
        nest.SetStatus(conn, {'weight': 18.})

        # nest.Simulate(t_pause)
        # timetot = timetot + t_pause

        nest.SetStatus(nest.GetConnections(source=neur_exc, target=neur_exc), {'alpha': _alpha_})  # era 5
        nest.SetStatus(neur_exc, {"b": _b_})  # 150.
        nest.SetStatus(neur_exc, {"tau_w": _tau_})  # 400.
        nest.SetStatus(neur_inp, {"C_m": 281.})

        # nest.SetStatus(neur_exc, {"Ca": 10.0})
        # nest.SetStatus(neur_exc, {"tau_Ca": 1000.0})

        nest.SetStatus(nest.GetConnections(source=neur_inp, target=neur_exc), {'lambda': 0.0})
        nest.SetStatus(nest.GetConnections(source=neur_exc, target=neur_out), {'lambda': 0.0})
        nest.SetStatus(nest.GetConnections(source=neur_exc, target=neur_exc), {'lambda': 0.0008})

        nest.SetStatus(nest.GetConnections(source=neur_inh, target=neur_exc),
                       {"weight": _Winh_})  # -0.5

        nest.SetStatus(nest.GetConnections(source=neur_exc, target=neur_inh), {"weight": 200.})  # -0.09???

        for inp in range(0, n_inp):
            for ex in range(0, n_exc * (step + 1)):
                w = nest.GetStatus(nest.GetConnections(source=[neur_inp[inp]], target=[neur_exc[ex]]))[0][
                    "weight"]  # -0.09???
                conn = nest.GetConnections(source=[neur_inp[inp]], target=[neur_exc[ex]])
                nest.SetStatus(conn, {'weight': w * 0.1})

        # switch off all teaching outputs
        for i in range(n_exc * (step + 1)):
            nest.SetStatus([noise_exc[i]], {"rate": _rate_})  # 650.
        ...
        for cycle in range(0, int(t_sleep), 100000):
                # simulation

            nest.Simulate(100000.)
            timetot = timetot + 100000.
            ...
            name = save_path + 'weights_PostSleep_' + str(int(cycle/100000)) + '.mat'
            sio.savemat(name, {'w_exc_exc': w_exc_exc})
        ...
        print '######################################################################'
        print '# Test post-sleep con context'
        print '######################################################################'
        print
        ...
        # Alla fine il risultato e'...
        #Accuracy_pre_sleep_Context[index] = float(count_right_num) / float(i_test + 1.) * 100.
        print 'Accuracy post sleep = ', float(count_right_num) / float(i_test + 1.) * 100., '%'
        print 'Accuracy_post_sleep_unsupervised_class =', float(count_unsupervised_class_right) / float(i_test + 1.) * 100.
        print 'Accuracy_post_sleep_unsupervised_group =', float(count_unsupervised_group_right) / float(i_test + 1.) * 100.
        print 'Accuracy KNN = ', float(count_KNN_right)/float(i_test+1.)*100., '%'
        print 'Accuracy pre sleep readout', AccPreReadout
        print 'Accuracy pre sleep class', AccPreClass
        print 'Accuracy pre sleep group', AccPreGroup
