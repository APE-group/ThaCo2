...
# Set save path
home = os.getcwd()
save_path = home +  '/Noise2/Mean'+ str(_Mean_) + '_std' + str(_Std_) + '/set' + str(_isubset_) + '/'
...
for i_cumul in range(0, N_RANKS * N_CLASSES, _interval_ * N_CLASSES):
    ...
    for i_train in range(i_cumul, i_cumul + _interval_ * N_CLASSES):
        ...
        #rate = np.random.normal(_Mean_, _Std_)
        #print 'rate', rate
    ...
    if i_cumul == 0:
        ...
        name = save_path + 'weights_PreSleep_' + str(step) + '.mat'
        ...
        nest.SetStatus(nest.GetConnections(source=neur_exc, target=neur_exc), {'alpha': 5.0})
        nest.SetStatus(neur_exc, {"b": 150.})
        nest.SetStatus(neur_exc, {"tau_w": 400.})
        ...
        print 'len conn exc exc', len(nest.GetConnections(source=neur_exc, target=neur_exc))

        print 'len inh', len(neur_inh)
        print 'len exc', len(neur_exc)
        print 'len conn inh exc', len(nest.GetConnections(source=neur_inh, target=neur_exc))

        # switch off all teaching outputs
        for i in range(n_exc * (step + 1)):
            nest.SetStatus([noise_exc[i]], {"rate": 650.})
        ...
        nest.Simulate(_t_sleep_)
        timetot = timetot + _t_sleep_
        ...
        name = save_path + 'weights_PostSleep_' + str(step) + '.mat'
