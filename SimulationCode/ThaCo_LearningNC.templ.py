...
# Set save path
home = os.getcwd()
save_path = home +  '/../SimulationOutput/ThaCo_LearningNC/set' + str(_isubset_) + '/'
...
for cl in range(0, N_CLASSES):
    feat_red = feat_train_class[cl][subset * N_RANKS:(subset + 1) *  N_RANKS]
    labels_red = label_train_class[cl][subset * N_RANKS: (subset + 1)* N_RANKS]
    rand = np.random.RandomState(cl)
    # Shuffle data
...
n_exc = n_exc_example * N_RANKS
n_inh = np.int32(20*10*40./4)#n_exc_example*N_CLASSES*N_RANKS / 4  # numero neuroni inibitori
...
n_spikes_exc = [0] * n_exc * int(N_CLASSES)
cum_spikes_exc = [0] * n_exc * int(N_CLASSES)
...
nest.Connect(neur_exc, sd_exc[0:n_exc], "one_to_one")
...
ClassPrediction = np.zeros([N_CLASSES, N_TEST])
GroupPrediction = np.zeros([N_CLASSES, N_TEST])
ReadoutPrediction = np.zeros([N_CLASSES, N_TEST])
ClassAccuracy_Arr = []
GroupAccuracy_Arr = []
ReadoutAccuracy_Arr = []
...
for cl in range(0,N_RANKS*N_CLASSES, N_RANKS):

    events_readout = []
    events_exc = []


    for i_train in range(cl, cl + N_RANKS):
